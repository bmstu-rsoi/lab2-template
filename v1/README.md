...
CREATE TABLE flight
(
    flightNumber   VARCHAR(80)              NOT NULL,
...
Если при покупке указан флаг `"painFromBalance": true`, то с бонусного счёта списываются максимальное количество
...
Если при покупке был указан флаг `"painFromBalance": false`, то в ответе будет `"paidByBonuses": 0`, а на бонусный
счет будет начислено бонусов в размере 10% от стоимости заказа. Так же в таблицу `privilegeHistory` будет добавлена
